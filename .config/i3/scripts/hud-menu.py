...
def try_appmenu_interface(window_id):
  ...
  gtk_window_name_cmd = subprocess.check_output(['xprop', '-id', window_id, '-notype', 'WM_CLASS']).decode('utf-8')
  name = gtk_window_name_cmd.split(' = ')[1].split(', ')
  ...
  if 'dmenu' in sys.argv:
    ...
    menu_cmd = subprocess.Popen(['rofi', '-dmenu', '-i', '-location', '1', '-width', '100', '-l', '15', '-scroll-method', '1', '-separator-style', 'solid', '-p', f"App Menu Bar for {name}"], stdout=subprocess.PIPE, stdin=subprocess.PIPE)
